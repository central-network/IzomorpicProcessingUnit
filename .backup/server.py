...
#          "IWldOkIdpLLzUXpVMDXZdTbrUHTAzpHVYndAXTWfWWU="
#
...
#         --origin-to-force-quic-on=cen.net:4433
#
#   4. Pass a flag to Chromium indicating which certificate needs to be trusted.
#      For the example above, that flag would be:
#         --ignore-certificate-errors-spki-list=IWldOkIdpLLzUXpVMDXZdTbrUHTAzpHVYndAXTWfWWU=
#
...
from aioquic.h3.events import H3Event, HeadersReceived, WebTransportStreamDataReceived, DatagramReceived, PushPromiseReceived, HeadersReceived, DataReceived
from aioquic.quic.configuration import QuicConfiguration
from aioquic.quic.connection import stream_is_unidirectional
from aioquic.quic.events import ProtocolNegotiated, StreamReset, QuicEvent

BIND_ADDRESS = 'cen.net'
BIND_PORT = 4433
...
class CounterHandler:
    ...
    def h3_event_received(self, event: H3Event) -> None:
        if isinstance(event, DatagramReceived):
            print(f"Received datagram: {event.data!r}")
            payload = str(len(event.data)).encode('ascii')
            self._http.send_datagram(self._session_id, payload)

        if isinstance(event, WebTransportStreamDataReceived):
            print(f"Received stream data on stream {event.stream_id}: {event.data!r}")
...
handler_remoteProcessorA = 0
handler_remoteProcessorB = 0
handler_jobDistributorAB = 0

http_remoteProcessorA = 0
http_remoteProcessorB = 0
http_jobDistributorAB = 0

ssid_remoteProcessorA = 0
ssid_remoteProcessorB = 0
ssid_jobDistributorAB = 0

conn_remoteProcessorA = 0
conn_remoteProcessorB = 0
conn_jobDistributorAB = 0

class CounterRemoteProcessorA:
    ...
    def __init__(self, session_id, http: H3Connection) -> None:
        ...
        http_remoteProcessorA = http
        ssid_remoteProcessorA = session_id
        conn_remoteProcessorA = http.create_webtransport_stream(session_id)
...
class CounterRemoteProcessorB:
    ...
    def __init__(self, session_id, http: H3Connection) -> None:
        ...
        http_remoteProcessorB = http
        ssid_remoteProcessorB = session_id
        conn_remoteProcessorB = http.create_webtransport_stream(session_id)
...
class CounterJobDistributorAB:
    ...
    def __init__(self, session_id, http: H3Connection) -> None:
        ...
        http_jobDistributorAB = http
        ssid_jobDistributorAB = session_id
        conn_jobDistributorAB = http.create_webtransport_stream(session_id)
    ...
    def h3_event_received(self, event: H3Event) -> None:
        ...
        if  isinstance(event, DatagramReceived):
            ...
            self._http.send_datagram(ssid_remoteProcessorA, payload)
            self._http.send_datagram(ssid_remoteProcessorB, payload)

        if  isinstance(event, HeadersReceived):
            print(f"HeadersReceived: {event.stream_id!r}")

        if  isinstance(event, PushPromiseReceived):
            print(f"PushPromiseReceived: {event.stream_id!r}")

        if  isinstance(event, DataReceived):
            print(f"DataReceived ({len(event.data)}): {event.stream_id!r}")
            self._http._quic.send_stream_data(conn_remoteProcessorA, str("asdasd").encode("ascii"), end_stream=False)
            self._http.send_data(ssid_remoteProcessorA, str("asdasd").encode("ascii"), end_stream=False)
            self._http.send_datagram(ssid_remoteProcessorB, str("asdasd").encode("ascii"))
            self._http._quic.datagrams_to_send(0.0)

        if  isinstance(event, WebTransportStreamDataReceived):
            print(f"Received stream data on stream {event.stream_id}: {event.data!r}")

            self._http.send_data(conn_jobDistributorAB, event.data, end_stream=False)
...
class WebTransportProtocol(QuicConnectionProtocol):
    ...
    def _handshake_webtransport(self, stream_id: int, request_headers: Dict[bytes, bytes]) -> None:
        ...
        if path == b"/remote-processor-a":
            assert(self._handler is None)
            self._handler = CounterRemoteProcessorA(stream_id, self._http)
            self._send_response(stream_id, 200)
            handler_remoteProcessorA = self._handler
            print(f"--> Remote processor A connected.")

        elif path == b"/remote-processor-b":
            assert(self._handler is None)
            self._handler = CounterRemoteProcessorB(stream_id, self._http)
            self._send_response(stream_id, 200)
            handler_remoteProcessorB = self._handler
            print(f"--> Remote processor B connected.")

        elif path == b"/job-distrubutor-ab":
            assert(self._handler is None)
            self._handler = CounterJobDistributorAB(stream_id, self._http)
            self._send_response(stream_id, 200)
            handler_jobDistributorAB = self._handler
            print(f"--> Job distributor connected.")
...
if __name__ == '__main__':
    ...
    try:
        logging.info("Listening on https://{}:{}".format(BIND_ADDRESS, BIND_PORT))
        loop.run_forever()
